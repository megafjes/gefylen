...
d3.select('body').append('div')
  ...
  .style('top', `${h/2 + l/2 - l/12 - l/40}px`) 
  .style('left', `${w/2 - l/2 + l/40}px`)
  .html(galaxy); 

d3.select('.galaxy svg')
  .attr('width', l/12)
  .attr('height', l/12);

var galaxyMap = d3.select('.galaxy')
  .append('svg')
  .attr('width', l/12)
  .attr('height', l/12)
  .attr('transform', `translate(${-l/12}, 0)`)
  .append('circle')
    .style('fill', "#fdb813")
    .attr('cx', l/24)
    .attr('cy', l/24 + l/35)
...
// d3.xml(galaxy).mimeType("image/svg+xml").get(function(error, xml) {
